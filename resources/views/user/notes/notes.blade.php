    ...
    <div class="container-fluid rounded-bottom bg-white py-3 px-3">
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr >
                    <td>
                        <a data-toggle="modal" data-target="#create_notes"  class="text-primary notes_title_content" data-name="{{$not->title??"Untitled Note"}}"  id="">{{$not->title??"Untitled Note"}}</a>
                    ...
                    <td>{!! $not->content ?? '' !!}</td>
                    <td>@if($not->note_file)<img src="{{ asset("storage/public/Files/$not->note_file") }}" width="60" height="60" alt="">@endif</td>
                    <td><a href="{{route('profile')}}">{{ Auth::user()->first_name }}</a></td>
                    <td>{{$not->updated_at}}</td>
                    <td>
                        <div class="d-flex justify-content-end">
                            <a data-toggle="modal" data-target="#create_notes"  class="text-primary notes_title_content" data-name="{{$not->title??"Untitled Note"}}" data-file="{{$not->note_file}}">
                            ...
                            <input type="hidden" value="{{route('edit_notes', [$not->id])}}" class="notes_action">
                            <input type="hidden" value="{{ route('delete_notes', [$not->id]) }}" class="notes_delete_hreff">
                            <a class="" href="{{ route('delete_notes', [$not->id]) }}">
